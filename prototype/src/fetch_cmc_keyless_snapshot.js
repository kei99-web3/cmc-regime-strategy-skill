...
const BASE_URL = "https://pro-api.coinmarketcap.com/trial-pro-api";
const TARGET_IDS = {
  BNB: 1839,
  CAKE: 7186,
  TWT: 5964
...
async function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { "user-agent": "cmc-regime-strategy-skill/0.1" } }, (response) => {
        let body = "";
        ...
        response.on("end", () => {
          if (response.statusCode < 200 || response.statusCode >= 300) {
            reject(new Error(`CMC keyless request failed: ${response.statusCode} ${body.slice(0, 240)}`));
            return;
          }
          try {
            resolve(JSON.parse(body));
          } catch (error) {
            reject(new Error(`CMC keyless JSON parse failed: ${error.message}`));
          }
        });
      })
      .on("error", reject);
...
async function buildKeylessSnapshot() {
  ...
  return {
    ...
    sourceModel: "CMC Keyless Public API trial endpoint; no API key, no signup, read-only market data.",
    sourceUrls: {
      quotes: quotesUrl,
...
async function main() {
  const snapshot = await buildKeylessSnapshot();
  const spec = generateStrategySpec(snapshot, { horizonDays: 7 });
  const replay = replayBacktestStub(spec);

  const prototypeRoot = path.resolve(__dirname, "..");
  fs.writeFileSync(path.join(prototypeRoot, "cmc_keyless_snapshot.json"), `${JSON.stringify(snapshot, null, 2)}\n`, "utf8");
  fs.writeFileSync(path.join(prototypeRoot, "cmc_keyless_strategy_fixture.json"), `${JSON.stringify(spec, null, 2)}\n`, "utf8");
  fs.writeFileSync(path.join(prototypeRoot, "cmc_keyless_replay_result.json"), `${JSON.stringify(replay, null, 2)}\n`, "utf8");

  console.log("fetch_cmc_keyless_snapshot.js: wrote keyless CMC snapshot, strategy fixture, and replay result.");
}
...
module.exports = { buildKeylessSnapshot };
